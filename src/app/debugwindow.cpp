...
DebugWindow::DebugWindow(GLWidget* glWidget, QWidget* parent) : QWidget(parent), m_glWidget(glWidget) {
    ...
    auto* msaa = option(renderLayout, "抗锯齿 MSAA", 0, 0, true); auto* mipmap = option(renderLayout, "纹理 Mipmap", 0, 1, false); m_wireframe = option(renderLayout, "线框叠加", 1, 0, false); auto* depth = option(renderLayout, "深度测试", 1, 1, true); auto* culling = option(renderLayout, "背面剔除", 2, 0, false); auto* srgb = option(renderLayout, "sRGB 输出", 2, 1, false);
    connect(msaa, &QCheckBox::toggled, glWidget, &GLWidget::setAntialiasing); connect(mipmap, &QCheckBox::toggled, glWidget, &GLWidget::setMipmaps); connect(m_wireframe, &QCheckBox::toggled, this, &DebugWindow::wireframeChanged); connect(depth, &QCheckBox::toggled, glWidget, &GLWidget::setDepthTest); connect(culling, &QCheckBox::toggled, glWidget, &GLWidget::setFaceCulling); connect(srgb, &QCheckBox::toggled, glWidget, &GLWidget::setSrgb); layout->addWidget(renderBox);

    auto* viewBox = new QGroupBox("调试视图", content); auto* viewLayout = new QGridLayout(viewBox); auto* debugView = new QComboBox(viewBox); debugView->addItems({"光照结果", "法线", "UV"}); viewLayout->addWidget(new QLabel("着色模式", viewBox), 0, 0); viewLayout->addWidget(debugView, 0, 1); connect(debugView, &QComboBox::currentIndexChanged, glWidget, &GLWidget::setDebugView); layout->addWidget(viewBox);
